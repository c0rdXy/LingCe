...
def center_window(window, width: int, height: int):
    ...
    try:
        ...
        ph = ctypes.windll.user32.GetSystemMetrics(1)  # ????
        if ph > 0:
            ratio = (r.b - r.t) / ph
    except Exception:
        pass
...
def center_window(window, width: int, height: int):
    ...
    window.update_idletasks()
    try:
        window.tk.eval("tk::PlaceWindow %s center" % window._w)
    except Exception:
...
def center_window(window, width: int, height: int):
    ...
    wa_x, wa_y, wa_w, wa_h = _get_work_area(window)

    x = max(0, wa_x + (wa_w - width) // 2)
    y = max(0, wa_y + (wa_h - height) // 2)
...
def center_window(window, width: int, height: int):
    ...
    # ????????????????????
    x = max(0, wa_x + (wa_w - width) // 2)
    y = max(0, wa_y + (wa_h - height - 40) // 2 + 40)
...
def center_window(window, width: int, height: int):
    ...
    # ???????????????????????/???
    window.geometry(f"{width}x{height}+{wa_x}+{wa_y}")
    window.update()

    # ????????
    decor_top = window.winfo_rooty() - window.winfo_y()
    decor_left = window.winfo_rootx() - window.winfo_x()

    # ????? = ??? + ??/???
    total_h = height + decor_top + decor_left  # ??? ? ???

    # ?????????????????
    x = max(0, wa_x + (wa_w - width - decor_left * 2) // 2)
    y = max(0, wa_y + (wa_h - total_h) // 2 + decor_top)
